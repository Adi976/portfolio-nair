...
void insertDynamic(int **CQ, int *front, int *rear, int *cnt)
{
    if (*cnt == QS)
    {
        // Reallocating twice the memory space
        *CQ = (int *)realloc(*CQ, sizeof(int) * (2 * QS));
        if (*CQ == NULL)
        {
            printf("Memory allocation failed\n");
            exit(1);
        }
        QS *= 2; // Update the global variable for the queue size
    }
